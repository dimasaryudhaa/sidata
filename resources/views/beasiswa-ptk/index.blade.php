...
<div class="container">
    ...
    @if($isPtk)
        <div class="table-responsive rounded-3 overflow-hidden mt-3">
            <table class="table table-bordered" id="beasiswaPtkTable">
                ...
                <tbody>
                    @foreach($beasiswaPtk as $index => $item)
                        <tr>
                            <td>{{ $beasiswaPtk->firstItem() + $index }}</td>
    ...
    const tbody = document.querySelector('#beasiswaPtkTable tbody');
    ...
    searchInput.addEventListener('keyup', function () {
        ...
        fetch(`/{{ $isAdmin ? 'admin' : 'ptk' }}/beasiswa-ptk/search?q=` + query)
            .then(res => res.json())
            .then(data => {
                tbody.innerHTML = '';
                ...
                data.forEach((item, index) => {
                    tbody.innerHTML += `
                        <tr>
                            ...
                            <td>
                                <a href="/{{ $isAdmin ? 'admin' : 'ptk' }}/beasiswa-ptk/${item.ptk_id}"
                                    class="btn btn-sm btn-no-border">
                                    <img src="/images/view.png" style="width:20px; height:20px;">
                                </a>

                                <a href="/{{ $isAdmin ? 'admin' : 'ptk' }}/beasiswa-ptk/create?ptk_id=${item.ptk_id}"
                                    class="btn btn-sm btn-no-border">
