...
<div class="container">
                ...
                <tbody>
                    @foreach($sertifikatPtk as $index => $item)
                        <tr>
                            ...
                            <td>
                                <a href="{{ route($prefix.'sertifikat-ptk.edit', ['sertifikat_ptk' => $item->sertifikat_id]) }}" class="btn btn-sm btn-no-border">
    ...
    @else
                ...
                <tbody>
                    @foreach($sertifikatPtk as $index => $item)
                        <tr>
                            ...
                            <td>
                                <a href="{{ route($prefix.'sertifikat-ptk.show', $item->ptk_id) }}" class="btn btn-sm btn-no-border">
                                    <img src="{{ asset('images/view.png') }}" alt="Lihat" style="width:20px;height:20px;">
                                </a>
                                <a href="{{ route($prefix.'sertifikat-ptk.create', ['ptk_id' => $item->ptk_id]) }}" class="btn btn-sm btn-no-border">
        ...
        const tbody = document.querySelector('#sertifikatPtkTable tbody');
        ...
        searchInput.addEventListener('keyup', function () {
            ...
            fetch(`/{{ $isAdmin ? 'admin' : 'ptk' }}/sertifikat-ptk/search?q=` + query)
                .then(res => res.json())
                .then(data => {
                    tbody.innerHTML = '';
                    ...
                    data.forEach((item, index) => {
                        tbody.innerHTML += `
                            <tr>
                                ...
                                <td>
                                    <a href="/{{ $isAdmin ? 'admin' : 'ptk' }}/sertifikat-ptk/${item.ptk_id}"
                                        class="btn btn-sm btn-no-border">
                                        <img src="/images/view.png" style="width:20px; height:20px;">
                                    </a>

                                    <a href="/{{ $isAdmin ? 'admin' : 'ptk' }}/sertifikat-ptk/create?ptk_id=${item.ptk_id}"
                                        class="btn btn-sm btn-no-border">
