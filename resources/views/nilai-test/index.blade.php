...
<div class="container">
    ...
    @else
        ...
        const tbody = document.querySelector('#nilaiTestTable tbody');
        ...
        searchInput.addEventListener('keyup', function () {
            ...
            fetch(`/{{ $isAdmin ? 'admin' : 'ptk' }}/nilai-test/search?q=` + query)
                .then(res => res.json())
                .then(data => {
                    tbody.innerHTML = '';
                    ...
                    data.forEach((item, index) => {
                        tbody.innerHTML += `
                            <tr>
                                ...
                                <td>${item.jumlah_nilai_test ?? 0}</td>
                                <td>
                                    <a href="/{{ $isAdmin ? 'admin' : 'ptk' }}/nilai-test/${item.ptk_id}"
                                        class="btn btn-sm btn-no-border">
                                        <img src="/images/view.png" style="width:20px; height:20px;">
                                    </a>

                                    <a href="/{{ $isAdmin ? 'admin' : 'ptk' }}/nilai-test/create?ptk_id=${item.ptk_id}"
                                        class="btn btn-sm btn-no-border">
