...
<div class="container">
    ...
    @else
        ...
        const tbody = document.querySelector('#tunjanganTable tbody');
        ...
        searchInput.addEventListener('keyup', function() {
            ...
            fetch(`/{{ $isAdmin ? 'admin' : 'ptk' }}/tunjangan/search?q=` + query)
                .then(res => res.json())
                .then(data => {
                    tbody.innerHTML = '';
                    ...
                    data.forEach((item, index) => {
                        tbody.innerHTML += `
                            <tr>
                                ...
                                <td>${item.jumlah_tunjangan ?? 0}</td>
                                <td>
                                    <a href="/{{ $isAdmin ? 'admin' : 'ptk' }}/tunjangan/${item.ptk_id}"
                                        class="btn btn-sm btn-no-border">
                                        <img src="/images/view.png" style="width:20px;">
                                    </a>

                                    <a href="/{{ $isAdmin ? 'admin' : 'ptk' }}/tunjangan/create?ptk_id=${item.ptk_id}"
                                        class="btn btn-sm btn-no-border">
