...
<div class="container">
    ...
    @else
        ...
        const tbody = document.querySelector('#penghargaanPtkTable tbody');
        ...
        searchInput.addEventListener('keyup', function () {
            ...
            fetch(`/{{ $isAdmin ? 'admin' : 'ptk' }}/penghargaan/search?q=` + query)
                .then(res => res.json())
                .then(data => {
                    tbody.innerHTML = '';
                    ...
                    data.forEach((item, index) => {
                        tbody.innerHTML += `
                            <tr>
                                ...
                                <td>
                                    <a href="/{{ $isAdmin ? 'admin' : 'ptk' }}/penghargaan/${item.ptk_id}"
                                        class="btn btn-sm btn-no-border">
                                        <img src="/images/view.png" style="width:20px; height:20px;">
                                    </a>

                                    <a href="/{{ $isAdmin ? 'admin' : 'ptk' }}/penghargaan/create?ptk_id=${item.ptk_id}"
                                        class="btn btn-sm btn-no-border">
