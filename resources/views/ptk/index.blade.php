    ...
    <div class="d-flex justify-content-between align-items-center mb-3">
        @if(!$isPtk)
            <div class="d-flex align-items-center">
                <a href="{{ route('admin.ptk.create') }}" class="btn btn-sm btn-no-border me-2">
                ...
                @foreach($ptk as $index => $p)
                    @if(!$isPtk)
                        <tr>
                            ...
                            <td>
                                <a href="{{ route($prefix.'.ptk.edit', $p->id) }}" class="btn btn-sm btn-no-border">
                                    <img src="{{ asset('images/edit.png') }}" alt="Edit" style="width:20px; height:20px;">
                                </a>

                                <form action="{{ route($prefix.'.ptk.destroy', $p->id) }}" method="POST" class="d-inline">
                                    @csrf
                                    @method('DELETE')
